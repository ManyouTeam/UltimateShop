...
public class ObjectItemConfig {
    ...
    public int getIntWithPAPI(Player player, String path, String defaultValue) {
        ...
        if (section.contains(path)) {
            return Integer.parseInt(TextUtil.withPAPI(section.getString(path), player));
        }
        return Integer.parseInt(TextUtil.withPAPI(shopSection.getString(path, defaultValue), player));
    }
