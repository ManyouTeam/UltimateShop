...
public class StaticPlaceholder {
    ...
    public static String getCompareValue(Player player, BigDecimal baseValue, BigDecimal compareValue) {
        ...
        if (compareValue.compareTo(baseValue) > 0) {
            return CommonUtil.modifyString(player, ConfigManager.configManager.getStringWithLang(player, "placeholder.compare.up", "↑"),
                    "base", baseValue.toString(), "compare", compareValue.toString());
        } else if (compareValue.compareTo(baseValue) == 0) {
            return CommonUtil.modifyString(player, ConfigManager.configManager.getStringWithLang(player, "placeholder.compare.same", "-"),
                    "base", baseValue.toString(), "compare", compareValue.toString());
        } else {
            return CommonUtil.modifyString(player, ConfigManager.configManager.getStringWithLang(player, "placeholder.compare.down", "↓"),
                    "base", baseValue.toString(), "compare", compareValue.toString());
        }
    }
}
